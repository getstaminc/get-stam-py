...
Revision ID: 72289ac8a760
Revises: eb148d61fdb7
Create Date: 2025-11-13 10:09:52.669150
...
revision: str = '72289ac8a760'
down_revision: Union[str, None] = 'eb148d61fdb7'
...
def upgrade() -> None:
    ...
    def get_historical_games(retries=4, delay=1):
        ...
        # Single SDQL query for all home-site games in the target date range.
        sdql_query = fields + f"@site='neutral' and date>=20250101 and date<20260101"
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(parsed, dict) and parsed.get('headers') and parsed.get('groups'):
                        ...
                        print(f"SDQL response not JSON: {e}; response snippet: {snippet}")
                        return []
    ...
    # We will run a single SDQL request for all home-site games in the
    # target date range and iterate the returned games. Keep the teams
    # lookup so we can resolve / insert team rows as we encounter new
    # canonical names in the results.
    print(f"Found {len(teams_dict)} existing NCAAB teams in database; fetching historical games with a single SDQL query for site='home'.")
    # pause variable retained for safety, though only one SDQL call is made
    ...
    for game in games:
            ...
            print(f"Inserting NCAAB game: {home_name} ({home_team_id}) vs {away_name} ({away_team_id}) on {game.get('date')} — score {game.get('points')}:{game.get('o:points')}")

            # Check if this exact game already exists OR if either team is already
            # playing on the same date (name-based check to catch mirrored neutral rows)
            ...
            if existing_game:
                print(f"Skipping duplicate or conflicting game: {home_name} vs {away_name} on {game.get('date')}")
                continue
