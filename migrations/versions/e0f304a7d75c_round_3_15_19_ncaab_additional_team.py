"""round 3 15-19 ncaab additional team

Revision ID: e0f304a7d75c
Revises: d80d4d98a71e
Create Date: 2025-11-06 20:43:19.891652
...
revision: str = 'e0f304a7d75c'
down_revision: Union[str, None] = 'd80d4d98a71e'
...
def upgrade() -> None:
    ...
    # But only process the teams that were created on 2025-11-05 (the set
    # of teams we want to seed games for).
    team_names = [
        row['team_name']
        for row in conn.execute(text("SELECT team_name FROM teams WHERE sport = 'NCAAB' AND created_at::date = '2025-11-05'")) .mappings()
    ]

    print(f"Found {len(teams_dict)} existing NCAAB teams in database; will process {len(team_names)} teams created on 2025-5.")
