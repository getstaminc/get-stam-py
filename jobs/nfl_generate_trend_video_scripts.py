...
def run(date_str=None, max_games=None, lookahead=False, time_pref=None):
    ...
    for entry in todays_trend_games:
        ...
        try:
            ...
            ranked = rank_game_trends(entry)
            ranked = annotate_live_series(ranked, "nfl", home_team, away_team, date_str)
            top_trends = ranked[:5]
            for t in top_trends:
                t["_score"] = round(get_confidence_score(t), 1)

            extra_context = gather_extra_context(entry, "nfl", game, home_team, away_team, tiktok=False)
            extra_context_tiktok = gather_extra_context(entry, "nfl", game, home_team, away_team, tiktok=True)
