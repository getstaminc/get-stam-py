"""
NCAAF Trend Video Script Generator

For every NCAAF game that would appear on the homepage's "Today's Trends"
...
Usage:
  venv/bin/python jobs/ncaaf_generate_trend_video_scripts.py [YYYY-MM-DD]
...
from api.services.historical.ncaaf_trends_service import NCAAFTrendsService
...
# Independent of MLB's own cap — set MAX_VIDEO_GAMES_PER_DAY_NCAAF to change.
MAX_VIDEO_GAMES_PER_DAY = int(os.getenv("MAX_VIDEO_GAMES_PER_DAY_NCAAF", "3"))

# NCAAF plays mostly Saturday, with some games sprinkled through the week —
# so a real "today" slate should generally be tried first (handled by run()
# below); on a date with no active-trend games, fall back to previewing the
# upcoming Saturday slate instead of producing nothing.
# python's date.weekday(): Monday=0 ... Sunday=6.
WEEKEND_FALLBACK_WEEKDAY = 5  # Saturday
...
SYSTEM_PROMPT = """You are a sports broadcast scriptwriter for GetSTAM, a sports betting trends platform. You write conversational, first-person spoken video scripts (90-130 seconds) for social media (TikTok/Reels/Shorts) that break down one upcoming NCAAF game the way a sharp, confident bettor would talk through their notes out loud — not like an ad, and not like a robotic stat recital.
...
TIKTOK_SYSTEM_PROMPT = """You are a sports broadcast scriptwriter for GetSTAM, a sports betting trends platform. You write conversational, first-person spoken video scripts (90-130 seconds) for TikTok that break down one upcoming NCAAF game the way a sharp, confident fan would talk through their notes out loud — not like an ad, and not like a robotic stat recital.
...
def build_user_prompt(game, ranked_trends, rankings):
    ...
    return f"""Upcoming NCAAF game: {away['team']} at {home['team']}, {game.get('commence_time')} ET.
...
def build_tiktok_user_prompt(game, ranked_trends, rankings):
    ...
    return f"""Upcoming NCAAF game: {game['away']['team']} at {game['home']['team']}, {game.get('commence_time')} ET.
...
def _fetch_trend_games(date_str):
    ...
    result, err = GameService.get_games_for_date("americanfootball_ncaaf", date_str)
    if err:
    ...
    trend_results, err = NCAAFTrendsService.analyze_multiple_games_trends(games, limit=20, min_trend_length=3)
    if err:
...
def run(date_str=None):
    ...
    if not todays_trend_games:
        ...
        print(f"No NCAAF games with active trends on {date_str} — trying the upcoming Saturday slate ({fallback_date}) instead.")
    ...
    if len(todays_trend_games) > MAX_VIDEO_GAMES_PER_DAY:
        print(
            ...
            f"(of {len(todays_trend_games)}) — set MAX_VIDEO_GAMES_PER_DAY_NCAAF to change this."
        )
        todays_trend_games = todays_trend_games[:MAX_VIDEO_GAMES_PER_DAY]

    try:
        rankings = fetch_ncaaf_rankings()
        # fetch_ncaaf_rankings() returns raw ESPN stat keys like "Total (Yds/G)"
        ...
        # what build_rankings_section expects (fetch_ncaaf_rankings returns two
        ...
        print(f"Warning: could not fetch NCAAF rankings ({e}); continuing without them.")
        rankings_by_team = {}
