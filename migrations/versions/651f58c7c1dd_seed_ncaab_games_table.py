"""seed ncaab games table

Revision ID: 651f58c7c1dd
Revises: 51541c18c4ff
Create Date: 2025-10-31 21:43:58.368216
...
revision: str = '651f58c7c1dd'
down_revision: Union[str, None] = '51541c18c4ff'
...
def upgrade() -> None:
    ...
    # Fetch teams for NCAAB
    teams_dict = {
        team['team_name']: team['team_id']
        for team in conn.execute(text("SELECT team_id, team_name FROM teams WHERE sport = 'NCAAB'")).mappings()
    }
    print(f"Found {len(teams_dict)} NCAAB teams in database.")

    # iterate over a snapshot of keys because we may insert new teams into
    # `teams_dict` during processing (resolve_team_id adds entries)
